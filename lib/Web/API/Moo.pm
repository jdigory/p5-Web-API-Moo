...
# ABSTRACT: Web::API::Moo - A simple base module (using Moo) to
# implement almost every RESTful API with just a few lines of
# configuration (forked from Web::API, which uses Mouse)
...
has user_agent => (
    is      => 'rw',
    isa     => Str,
    default => sub { __PACKAGE__ . ' ' . $Web::API::Moo::VERSION },
);
...
has json => (
    is      => 'rw',
    #isa     => InstanceOf['JSON::MaybeXS::JSON()'],
    isa     => sub { JSON::MaybeXS::JSON() },
    default => sub {
        JSON::MaybeXS->new(
            utf8            => 1,
            allow_blessed   => 1,
...
sub encode {
    ...
    my $payload;
    eval {
        # custom encoder should only be run if called by Web::API::Moo otherwise we
        # end up calling it twice
        if ($self->has_encoder and caller(1) eq 'Web::API::Moo') {
            $self->log('running custom encoder') if $self->debug;
            $payload = $self->encoder->($options, $content_type);
        }
        else {
            for ($content_type) {
                if (/plain/) { $payload = $options; }
                elsif (/urlencoded/) {
                    $payload .=
...
sub key_exists {
    ...
    return $node
        if @bits == grep { ref $node eq 'HASH' && exists $node->{$_} && ( $node = $node->{$_} // {} ) } @bits;
...
sub needs_retry {
    ...
    return 1 if match ( $response->code, $self->retry_http_codes );
...
Please report any bugs or feature requests on GitHub's issue tracker L<https://github.com/jdigory/p5-Web-API-Moo/issues>.
...
L<https://github.com/jdigory/p5-Web-API-Moo>

=item * MetaCPAN

L<https://metacpan.org/module/Web::API::Moo>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/Web::API::Moo>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/Web::API::Moo>

=back

=head1 SEE ALSO

L<Web::API>, L<HTTP::Cookies>, L<LWP::UserAgent>, L<Net::OAuth>
